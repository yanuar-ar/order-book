...
- **INV-ARI-07 — Filter order (tick/lot/notional).** Per-market filter statis (CEX-style) divalidasi saat submit: `price` kelipatan `tickSize` dan dalam `[minPrice, maxPrice]`; `qty` kelipatan `stepSize` dan dalam `[minQty, maxQty]` (lot pasar terpisah untuk Market/Stop-Market); `Notional(price, qty)` dalam `[minNotional, maxNotional]` (order pasar memakai harga trade terakhir sebagai referensi, *fail-open* sebelum trade pertama; display iceberg juga harus on-step dan ≥ `minQty`). Order melanggar → **ditolak** (alasan per-grup), bukan dibulatkan diam-diam, tanpa mutasi state. Sebagai invarian *resting*: setiap order yang nge-rest harus on-grid (harga on-tick & in-range, sisa/`display` on-step) — **minimum tidak diasserkan untuk sisa** karena partial fill boleh menyisakan remainder di bawah `minQty`/`minNotional`. Fuzz input off-tick/off-lot/off-notional dan amend-down ke bawah grid.
