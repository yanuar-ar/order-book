...
func WriteSnapshot(path string, seq uint64, sections [][]byte) error {
	body := encodeSnapshot(seq, sections)
	tmp := path + ".tmp"
...
func ReadSnapshot(path string) (seq uint64, sections [][]byte, err error) {
...
func encodeSnapshot(seq uint64, sections [][]byte) []byte {
	// Layout: magic(4) seq(8) nSections(4) [len(4) bytes]... crc(4 over preceding).
	size := 16
