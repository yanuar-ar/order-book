...
for the engine default). Replication keys: `OB_REPLICATION_MODE` (`off` default,
or `sync`/`async` hot-standby) and `OB_REPLICATION_RING` (power of two or 0). Snapshot keys: `OB_SNAPSHOT_PATH` (must differ from
